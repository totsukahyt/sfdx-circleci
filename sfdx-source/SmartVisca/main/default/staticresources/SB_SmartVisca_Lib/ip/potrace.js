...
var Potrace = (function() {
	...
	function setData(imgdataobj, width, height) {
		if (info.isReady) {
			clear();
		}
	...
	function bmToPathlist() {
		...
		function findPath(point) {
			var path = new Path(),
				x = point.x, y = point.y, dirx = 0, diry = 1, tmp;
		...
		function adjustVertices(path) {
			...
			function pointslope(path, i, j, ctr, dir) {
				...
				var n = path.len, sums = path.sums,
					x, y, x2, xy, y2, k, a, b, c, lambda2, l, r = 0;
	...
	function getSVG(size, opt_type) {
		...
		var w = bm.w * size, h = bm.h * size, len = pathlist.length, c, i, strokec, fillc, fillrule;
...
function getSVGdata(data, width, height) {
	Potrace.setData(data, width, height);

	Potrace.process(function() {
		displaySVG(width, height, 1);
	});
}

function displaySVG(width, height, size, type) {
	var svgStr = Potrace.getSVG(size, type);
	var d = svg_data(svgStr);

// canvasへの描画
/*
	ctx.clearRect(0, 0, width, height);
	ctx.beginPath(); // コンテクストのパスを初期化する
	ctx.fillStyle = "#FFFFFF";
	ctx.fillRect(0, 0, width, height);
	ctx.lineWidth = 0.5;
*/
	var wv = Math.floor(width / 10);
	var hv = Math.floor(height / 10);
	var str = "";
	var strs = "";
	var flag = false;
	var nn = 0;
	for (var i = 0; i < d.data.length; i++) {
		var ddd = d.data[i];
		var d_d = ddd.data;
		for (var j = 0; j < d_d.length; j++) {
			if (d_d[j].c == 'M') {
				if (d_d[j].d[0] == 0 && d_d[j].d[1]== 240) {
					flag = true;
					break;//なんでかな
				}
				str += "M" + d_d[j].d[0] + " " + d_d[j].d[1] + " ";
				strs += "M" + d_d[j].d[0] + "," + d_d[j].d[1] + "," + d_d[j].a + "," + d_d[j].b + "," + d_d[j].di + ",";
				xv = Math.floor(d_d[j].d[0] / 10);
				yv = Math.floor(d_d[j].d[1] / 10);
			}
			else if (d_d[j].c == 'L') {
				str += "L " + d_d[j].d[0] + " " + d_d[j].d[1] + " ";
			}
		}
	}
//	ctx.stroke();// 画面上に描画する
...
//console.log("flag " + JSON.stringify(d));
...
//svg stringからpathデータに変換する
function svg_data(str) {
	s = str.substring(str.indexOf('<path d="') + ('<path d="').length);
	...
	for (var i = 0; i < s.length; i++) {
		...
		if ((c >= 'a' && c <= 'z') ||(c >= 'A' && c <= 'Z')) {
			...
			np = i + 1;
			var j;
			for (j = np; j < s.length; j++) {
				c = s.charAt(j);
				if ((c >= 'a' && c <= 'z') ||(c >= 'A' && c <= 'Z')) break;
			}
			nu[nn] = {};
			nu[nn].c = com;
			d = s.substring(np, j).trim();
			d = d.replaceAll(',', ' ');
			nu[nn].d = d.split(' ');
			nn++;
			i = j - 1;
	...
	for (var i = 0; i < nu.length; i++) {
		if (nu[i].c == 'M') {
			...
			var y = new Array();
			var n = 0;
			x[n] = parseFloat(nu[i].d[0]);
			y[n++] = parseFloat(nu[i].d[1]);
			var j;
			for (j = i + 1; j < nu.length; j++) {
				if (nu[j].c == 'M') break;
				x[n] = parseFloat(nu[j].d[nu[j].d.length - 2]);
				y[n++] = parseFloat(nu[j].d[nu[j].d.length - 1]);
			}
			...
			nuu = checkDist(x, y);
			if (nuu == null || nuu.data.length == 0) continue;
			data.push(nuu);
			ss += nuu.str;
		}
	}
//console.log("svg_data x " + JSON.stringify(x));
//console.log("svg_data y " + JSON.stringify(y));
//console.log("svg_data " + JSON.stringify(data));
//	meishiV = {str: ss, data: data};
//alert(JSON.stringify(data));
	return {str: ss, data: data};
}

//最大値、最小値を求め、長い直線を求める
//moveデータにa（傾き）、b（接点）、di（長さ）、dx、dy（幅、高さ）を加える
//連続する描画要素の最初と最後をLで結ぶ
function checkDist(x, y) {
	...
	var dx = xmax - xmin;//大きさ
	var dy = ymax - ymin;

	var dist = Math.sqrt(dx * dx + dy * dy);
	if (dist < 10) return null;//四角形として見た場合の大きさ

	dx = Math.abs(dx);
	dy = Math.abs(dy);
	if (dx < dy) dd = dx / dy;
	else dd = dy / dx;
	if (dd > 0.75) return null;//直線の角度
...
//		if(dist > 60) {
		if(dist > 10) {
			var m = lineExp(x[i], y[i], x[i + 1], y[i + 1]);
//console.log("dist=" + dist + " x=" + x[i + 1] + " y=" + y[i + 1] + " a=" + m.a + " b=" + m.b);

			s += "M" + x[i] + " " + y[i] + " L " + x[i + 1] + " " + y[i + 1] + " ";

			if (x[i] == 0 && y[i] == 240) return null;
			if (x[i] == 0 && y[i] == 180) return null;
			...
			val[nn].b = m.b;
			val[nn].di = dist;
			val[nn].dx = Math.abs(dx);
			val[nn].dy = Math.abs(dy);
			val[nn++].d = [x[i], y[i], x[i + 1], y[i + 1]];

			val[nn] = {};
			val[nn].c = "L";
			val[nn++].d = [x[i + 1], y[i + 1]];
		}
	}
	return {str: s, data: val};
}

function direction(x, y, xmin, ymin, xmax, ymax, dx2, dy2) {
	var d = {};
	d.c = 'L';
	if (x >= xmin && x < xmin + dx2) {
		if (y >= ymin && y < ymin + dy2) {
			d.d = [xmin, ymin, xmax, ymax];
		}
		else {
			d.d = [xmin, ymax, xmax, ymin];
		}
	}
	else {
		if (y >= ymin && y < ymin + dy2) {
			d.d = [xmax, ymin, xmin, ymax];
		}
		else {
			d.d = [xmax, ymax, xmin, ymin];
		}
	}
	return d;
}
...
function lineExp(x0, y0, x1, y1) {
	...
	m.dist = Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));
...
function variance(data) {
	...
	return {ave: ave, vari: (varia / data.length)};
}

/*
 * 標準偏差を求める
 */
//戻り値　v.ave：平均値、v.vari：分散、v.sd：標準偏差、v.coe：変動係数
function standard_deviation(data) {
	// 分散を求める
	var v = variance(data);
	var sd = Math.sqrt(v.vari);
	var coe = sd / v.ave;

	// 分散の平方根
	return {ave: v.ave, vari: v.vari, sd: sd, coe: coe};
}
